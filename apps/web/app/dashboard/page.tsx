...
import {
  ...
  ChevronDown, ChevronUp, Paperclip, Trash2, FileText,
} from 'lucide-react';
...
function ReportExportButton({ filters }: { filters?: { dateFrom?: string; dateTo?: string; status?: string; customer?: string } }) {
  ...
  return (
    <Button size="sm" variant="outline" onClick={handleDownload} disabled={downloading}>
      <FileSpreadsheet className="mr-1.5 h-3.5 w-3.5" />{downloading ? '다운로드 중...' : '보고서'}
    </Button>
  );
}
...
/* ── 정렬 가능 헤더 (대시보드) ── */
function DashSortTh({ label, sortKey: sk, currentKey, dir, onSort, className = '' }: {
    ...
    <div className="overflow-hidden rounded-2xl border border-white/60 bg-white/70 shadow-sm backdrop-blur-xl dark:border-gray-800/60 dark:bg-gray-900/70">
      ...
      <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {!isTier2 && (
            <Button size="sm" variant="outline" onClick={handleExcelExport} disabled={exporting || sorted.length === 0}>
              <FileSpreadsheet className="mr-1.5 h-3.5 w-3.5" />
              {exporting ? '내보내기...' : `목록 CSV (${sorted.length}건)`}
            </Button>
          )}
      ...
      {/* 필터 */}
      <div className="overflow-x-auto border-t border-gray-100 bg-gray-50/30 dark:border-gray-800 dark:bg-gray-800/20">
        <div className="flex flex-wrap items-center gap-2 px-3 py-2.5 sm:px-4 sm:py-3">
          ...
          {hasFilters && (
            <button onClick={resetAll} className="text-xs text-primary hover:underline whitespace-nowrap">초기화</button>
          )}
          <span className="ml-auto text-xs text-muted-foreground whitespace-nowrap">{sorted.length}건</span>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          {hasFilters && <button onClick={resetAll} className="mt-2 text-xs text-primary hover:underline">필터 초기화</button>}
        </div>
      ) : (
          ...
          <table className="w-full min-w-[1600px] text-xs border-collapse">
            <thead className="sticky top-0 z-10">
              ...
              <tr className="border-b border-gray-200 bg-gray-50/80 dark:border-gray-700 dark:bg-gray-800/40">
                ...
                <DashSortTh label="발생일" sortKey="occurredDate" currentKey={sortKey} dir={sortDir} onSort={toggleSort} className="text-blue-500/80" />
                <DashSortTh label="대분류" sortKey="class" currentKey={sortKey} dir={sortDir} onSort={toggleSort} className="text-blue-500/80" />
                <DashSortTh label="중분류" sortKey="category" currentKey={sortKey} dir={sortDir} onSort={toggleSort} className="text-blue-500/80" />
                <DashSortTh label="세부항목" sortKey="item" currentKey={sortKey} dir={sortDir} onSort={toggleSort} className="text-blue-500/80" />
                <DashSortTh label="발생부서" sortKey="department" currentKey={sortKey} dir={sortDir} onSort={toggleSort} className="text-blue-500/80" />
                <DashSortTh label="담당자" sortKey="manager" currentKey={sortKey} dir={sortDir} onSort={toggleSort} className="text-blue-500/80 border-r border-gray-200" />
                <DashSortTh label="고객사" sortKey="customer" currentKey={sortKey} dir={sortDir} onSort={toggleSort} className="text-gray-400" />
                <DashSortTh label="프로젝트" sortKey="project" currentKey={sortKey} dir={sortDir} onSort={toggleSort} className="text-gray-400" />
                <DashSortTh label="공장" sortKey="factory" currentKey={sortKey} dir={sortDir} onSort={toggleSort} className="text-gray-400" />
                <th className="whitespace-nowrap px-2 py-1.5 text-center text-[9px] font-semibold text-gray-400">품번</th>
                <DashSortTh label="협력사" sortKey="company" currentKey={sortKey} dir={sortDir} onSort={toggleSort} className="text-gray-400 border-r border-gray-200" />
                <DashSortTh label="조치시점" sortKey="actionDate" currentKey={sortKey} dir={sortDir} onSort={toggleSort} className="text-amber-500/80" />
